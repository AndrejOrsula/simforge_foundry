...
def cubesat_node_group():
    ...
    geometry_socket = cubesat.interface.new_socket(
        name="geometry", in_out="OUTPUT", socket_type="NodeSocketGeometry"
    )
    geometry_socket.attribute_domain = "POINT"

    seed_socket = cubesat.interface.new_socket(
    ...
    rail_size_socket = cubesat.interface.new_socket(
        name="rail_size", in_out="INPUT", socket_type="NodeSocketFloat"
    )
    rail_size_socket.default_value = 0.004999999888241291
    rail_size_socket.min_value = 0.0
    rail_size_socket.max_value = 3.4028234663852886e38
    rail_size_socket.subtype = "DISTANCE"
    rail_size_socket.attribute_domain = "POINT"
    rail_size_socket.force_non_field = True

    rail_end_length_socket = cubesat.interface.new_socket(
        name="rail_end_length", in_out="INPUT", socket_type="NodeSocketFloat"
    )
    rail_end_length_socket.default_value = 0.009999999776482582
    rail_end_length_socket.min_value = 0.0
    rail_end_length_socket.max_value = 3.4028234663852886e38
    ...
    border_size_socket = cubesat.interface.new_socket(
    ...
    cells_mat1_socket = cubesat.interface.new_socket(
        name="cells_mat1", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    cells_mat1_socket.attribute_domain = "POINT"

    cells_mat2_socket = cubesat.interface.new_socket(
        name="cells_mat2", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    cells_mat2_socket.attribute_domain = "POINT"

    cells_mat3_socket = cubesat.interface.new_socket(
        name="cells_mat3", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    cells_mat3_socket.attribute_domain = "POINT"

    frame_mat1_socket = cubesat.interface.new_socket(
        name="frame_mat1", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    frame_mat1_socket.attribute_domain = "POINT"

    frame_mat2_socket = cubesat.interface.new_socket(
        name="frame_mat2", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    frame_mat2_socket.attribute_domain = "POINT"

    frame_mat3_socket = cubesat.interface.new_socket(
        name="frame_mat3", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    frame_mat3_socket.attribute_domain = "POINT"

    frame_mat4_socket = cubesat.interface.new_socket(
        name="frame_mat4", in_out="INPUT", socket_type="NodeSocketMaterial"
    )
    ...
    vector_math.inputs[1].default_value = (
        1.5707963705062866,
        1.5707963705062866,
        1.5707963705062866,
    )
    ...
    random_value_001.inputs[0].default_value = (
        -3.1415927410125732,
        -3.1415927410125732,
        -3.1415927410125732,
    )
    random_value_001.inputs[1].default_value = (
        3.1415927410125732,
        3.1415927410125732,
        3.1415927410125732,
    )
    ...
    distribute_points_on_faces = cubesat.nodes.new(
    ...
    cubesat.links.new(
        distribute_points_on_faces.outputs[0], instance_on_points_004.inputs[0]
    )
    cubesat.links.new(instance_on_points_004.outputs[0], join_geometry_005.inputs[0])
    cubesat.links.new(
        align_rotation_to_vector.outputs[0], instance_on_points_004.inputs[5]
    )
    cubesat.links.new(normal_004.outputs[0], capture_attribute_001.inputs[1])
    cubesat.links.new(
        capture_attribute_001.outputs[1], align_rotation_to_vector.inputs[2]
    )
