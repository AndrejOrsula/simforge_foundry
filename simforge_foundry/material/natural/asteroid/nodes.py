...
def asteroidshader_node_group():
    asteroidshader = bpy.data.node_groups.new(
        type="ShaderNodeTree", name="AsteroidShader"
    )
    ...
    bsdf_socket = asteroidshader.interface.new_socket(
        name="BSDF", in_out="OUTPUT", socket_type="NodeSocketShader"
    )
    bsdf_socket.attribute_domain = "POINT"

    scale_socket = asteroidshader.interface.new_socket(
    ...
    color1_socket = asteroidshader.interface.new_socket(
    ...
    rgb_curves_curve_3_point_2 = rgb_curves_curve_3.points.new(
        0.7045456171035767, 0.9687498211860657
    )
